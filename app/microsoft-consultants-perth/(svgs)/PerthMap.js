// app/microsoft-consultants-perth/(svgs)/PerthMap.jsx
import React from "react";

// ─────────────────────────────────────────────────────────────
// ViewBox: 500 × 620  (identical to MelbourneMap)
// Perth replaces Melbourne as the primary (highlighted) city.
// All other cities, connections and HQ node are unchanged.
// ─────────────────────────────────────────────────────────────
...
const cities = [
  ...
  // ── Richmond — tertiary near Melbourne ───────────────────
  {
    id: "richmond",
    label: "Richmond VIC",
    x: 190,
  ...
  // ── Eastern seaboard — top to bottom ─────────────────────
  {
    id: "cairns",
    label: "Cairns",
    x: 448,
  ...
  // ── Melbourne — secondary (was primary in MelbourneMap) ───
  {
    id: "melbourne",
    label: "Melbourne",
    x: 160,
...
// Perth coordinates (matches the primary city entry above)
const PERTH = { x: 38, y: 380 };
...
const PerthMap = () => (
  ...
  >
    <defs>
      ...
      <radialGradient id="perth-glow" cx="50%" cy="50%" r="50%">
    ...
    <circle cx={PERTH.x} cy={PERTH.y} r="56" fill="url(#perth-glow)" />
    ...
    <circle
      ...
      strokeWidth="1"
      strokeOpacity="0.35"
      style={{ animation: "pulse-perth 2.8s ease-out 0.5s infinite" }}
    />
    ...
    <g className="city-node" style={{ animationDelay: "0.15s" }}>
      <circle
        ...
        strokeWidth="1.5"
      />
      <circle cx={PERTH.x} cy={PERTH.y} r={DR.primary} fill="#046999" />
      <text
        x={PERTH.x}
        y={PERTH.y + R.primary + 14}
        textAnchor="middle"
        fontFamily="system-ui, -apple-system, sans-serif"
        fontSize={FS.primary}
        fontWeight="700"
