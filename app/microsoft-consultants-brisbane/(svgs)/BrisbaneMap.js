// app/microsoft-consultants-brisbane/(components)/BrisbaneMap.js
import React from "react";

// ─────────────────────────────────────────────────────────────
// ViewBox: 500 × 620
// Adapted from SydneyMap — primary node changed to Brisbane,
// Sydney demoted to secondary, Gold Coast added as tertiary neighbour.
// ─────────────────────────────────────────────────────────────

const HQ = { x: 440, y: 310 };

// Brisbane sits at x: 448, y: 192 on the eastern seaboard
const BRISBANE = { x: 448, y: 192 };
...
const cities = [
  ...
  // ── Page subject — Brisbane as primary ───────────────────
  {
    id: "brisbane",
    label: "Brisbane",
...
const BrisbaneMap = () => (
  ...
  >
    <defs>
      ...
      <radialGradient id="bne-glow" cx="50%" cy="50%" r="50%">
    ...
    <circle cx={BRISBANE.x} cy={BRISBANE.y} r="56" fill="url(#bne-glow)" />
    ...
    <circle
      ...
      strokeWidth="1"
      strokeOpacity="0.35"
      style={{ animation: "pulse-bne 2.8s ease-out 0.5s infinite" }}
    />
    ...
    <g className="city-node" style={{ animationDelay: "0.15s" }}>
      <circle
        ...
        strokeWidth="1.5"
      />
      <circle cx={BRISBANE.x} cy={BRISBANE.y} r={DR.primary} fill="#046999" />
      {/* Label above the node — Brisbane sits near the top so label goes below */}
      <text
        x={BRISBANE.x}
        y={BRISBANE.y + R.primary + 14}
        textAnchor="middle"
        fontFamily="system-ui, -apple-system, sans-serif"
        fontSize={FS.primary}
        fontWeight="700"
