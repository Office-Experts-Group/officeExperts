...
7. Update file properties
  ...
  const articleSchema = {
    ...
    description:
      "Learn how to export data from Power Apps to PDF using Power Automate. This tutorial shows the easiest and most effective way to generate professional PDF documents.",
    mainEntityOfPage: {
      "@type": "WebPage",
      "@id":
        "https://www.powerplatformexperts.com.au/blog/export-to-pdf-power-apps",
    },
    ...
    video: {
      ...
      description:
        "Exporting data to PDF from Power Apps is a common requirement in business applications. Learn the streamlined approach using Power Automate.",
    ...
    <>
      ...
      <div className={styles.blogContainer}>
        ...
        <div className={styles.blogContent}>
          <h2>Introduction</h2>
          <p>
            Exporting data to PDF from Power Apps is a common requirement in
            business applications. While Power Apps doesn't have a built-in
            "Export to PDF" function, we can achieve this functionality by
            combining Power Apps with Power Automate (Flow) to create
            professional-looking PDF exports of your app data.
          </p>
          <p>
            In this guide, I'll show you a streamlined approach to PDF exports,
            including how to:
          </p>
          <ul>
            <li>
              Set up a Flow triggered when a record is saved in SharePoint
            </li>
          ...
          <div className={styles.videoContainer}>
            <iframe
              src="https://www.youtube.com/embed/_mgLYllGY-Y?si=3OuyiaFX5r1sfORy"
              width="560"
              height="315"
          ...
          <p>
            For this tutorial, we'll use a simple "Issues Tracker" Power App
            that logs issues to a SharePoint list. When a new issue is created,
            our Flow will automatically generate a PDF export of that issue and
            attach it to the SharePoint item.
          </p>
          <p>
          ...
          <ul>
            <li>
              <strong>Title</strong> - The name of the issue
            </li>
            <li>
          ...
          <p>
            Let's start by creating a Flow that will handle the PDF export
            process. This Flow will trigger whenever a new item is created in
            our SharePoint list.
          </p>

          <h3>Step 1: Create a New Cloud Flow</h3>
          <p>
            Start by creating a new cloud flow in Power Automate with a
            SharePoint trigger:
          </p>
          <ol>
            <li>
              Navigate to Power Automate and select "Create" {">"} "Cloud flow"
            </li>
          ...
          <p>
            If you want to include images in your PDF (like a company logo),
            you'll need to retrieve that image file first:
          </p>
          <ol>
          ...
          <p>
            This step retrieves the binary content of your image file, which
            we'll encode into the HTML later.
          </p>
          ...
          <ol>
            ...
            <li>
              In the "Inputs" field, create the HTML structure using values from
              your SharePoint item
            </li>
          </ol>
          <p>For a basic structure with just text, you could use:</p>

          <CodeBlock code={htmlStructureCode} language="html" />

          <p>
            The tricky part is adding an image. To include an image in your PDF,
            you'll need to encode it as a base64 string directly in your HTML:
          </p>

          <CodeBlock code={imageTagCode} language="html" />

          <p>Combining these, your complete HTML might look like:</p>

          <CodeBlock code={completeHtmlCode} language="html" />

          <h3>Step 4: Create a Temporary HTML File</h3>
          <p>Before converting to PDF, we need to save our HTML as a file:</p>
          <ol>
            <li>Add a "Create file" action for OneDrive</li>
            <li>
              Select the root folder of your OneDrive (or any folder you prefer)
            </li>
            <li>
              For the file name, use a dynamic expression:{" "}
              <code>concat(triggerOutputs()?['body/Title'], '.html')</code>
            </li>
            <li>For the file content, use the output from your Compose step</li>
          </ol>
          <p>
            This creates a temporary HTML file in OneDrive, which we'll then
            convert to PDF.
          </p>

          <h3>Step 5: Convert the HTML File to PDF</h3>
          <p>Now comes the magic step - converting our HTML to PDF:</p>
          <ol>
            <li>Add a "Convert file" action from OneDrive</li>
            <li>
              For the "File", select the ID from the previous "Create file" step
            </li>
            <li>Set the target format to "PDF"</li>
          </ol>
          <p>
            This step handles the conversion from HTML to PDF, giving us a
            properly formatted document.
          </p>

          <h3>Step 6: Save the PDF to SharePoint</h3>
          <p>
            Now that we have our PDF, let's save it to SharePoint where our
            Power App can access it:
          </p>
          <ol>
            <li>Add a "Create file" action for SharePoint</li>
            <li>
              Select your SharePoint site and a document library (e.g., "Issues
              Attachments")
            </li>
            <li>For the file name, use the file name from the Convert step</li>
            <li>
              For the file content, use the file content from the Convert step
            </li>
          </ol>

          <h3>Step 7: Link the PDF to Your SharePoint Item</h3>
          <p>
            Finally, we need to associate this PDF with our original SharePoint
            item:
          </p>
          <ol>
            ...
            <li>
              Add a property: "ParentID" = ID from the trigger (SharePoint list
              item ID)
            </li>
          </ol>
          <p>
            This creates a relationship between your PDF and the SharePoint
            item, making it appear as an attachment.
          </p>

          <h3>Complete Flow Structure</h3>
          <p>Here's a summary of the complete flow:</p>

          <CodeBlock code={powerAutomateCode} language="javascript" />

          <h2>Testing the Solution</h2>
          <p>Once your flow is set up, it's time to test the solution:</p>
          <ol>
            ...
            <li>
              After the flow completes, you should see a PDF attachment on your
              SharePoint item
            </li>
          </ol>
          <p>
            Opening the PDF should show your formatted content, including any
            images you've included.
          </p>

          <h2>Customising Your PDF Output</h2>
          <p>You can enhance your PDF output in several ways:</p>
          <ul>
            <li>
              <strong>Add more fields:</strong> Include any field from your
              SharePoint item in the HTML
            </li>
            <li>
              <strong>Improve styling:</strong> Add CSS to your HTML to control
              fonts, colors, and layout
            </li>
            <li>
              <strong>Add tables:</strong> Format data in tables for better
              readability
            </li>
            <li>
              <strong>Add conditional content:</strong> Use Power Automate
              expressions to show/hide content based on conditions
            </li>
          </ul>
          <p>
            Remember that you're essentially creating a web page that will be
            converted to PDF, so most HTML and CSS techniques will work.
          </p>

          <h2>Key Challenges and Solutions</h2>
          <h3>Including Images</h3>
          <p>
            The most challenging part of this solution is including images in
            your PDF. The key is to:
          </p>
          <ol>
            <li>Retrieve the image file using "Get file content"</li>
            <li>
              Encode the image as a base64 string within an HTML image tag
            </li>
            <li>Include the content type to ensure proper rendering</li>
          </ol>
          <p>
            This approach works for most image types (PNG, JPEG, etc.) but be
            mindful of file sizes, as large images will increase processing
            time.
          </p>

          <h3>HTML Formatting</h3>
          <p>For best results with the PDF conversion:</p>
          <ul>
          ...
          <h2>Conclusion</h2>
          <p>
            Exporting to PDF from Power Apps is straightforward once you
            understand the process. By leveraging Power Automate, we can create
            professional-looking PDF exports that include both text and images,
            providing a complete solution for document generation.
          </p>
          ...
          <p>
            With these tools, you can enhance your Power Apps solutions with
            professional document generation capabilities, meeting the needs of
            business users who require formatted outputs from their
            applications.
          </p>

          <p>
            If you'd like to see more Power Platform development tutorials,
