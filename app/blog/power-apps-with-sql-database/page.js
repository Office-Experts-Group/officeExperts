...
// - ThisItem.OrderDate`;
  ...
  const articleSchema = {
    ...
    description:
      "Learn how to use SharePoint lists as a relational database in Power Apps. This guide shows you how to implement parent-child relationships and create connected data structures without needing SQL Server or Dataverse licenses.",
    mainEntityOfPage: {
      "@type": "WebPage",
    ...
    <>
      ...
      <div className={styles.blogContainer}>
        ...
        <div className={styles.blogContent}>
          <h2>Introduction</h2>
          <p>
            Just as our prehistoric ancestors evolved by adopting tools like
            fire and stone axes, your journey as a Power Apps developer must
            also evolve. As your experience grows, you'll need to reach further
            and build more complex applications. One of the most significant
            milestones in this evolution is moving beyond SharePoint lists and
            connecting your apps to enterprise-level storage solutions like
            Microsoft SQL Server.
          </p>
          <p>In this guide, I'll walk you through the process of:</p>
          <ul>
            <li>Setting up an Azure SQL database</li>
            <li>Creating tables and relationships</li>
            <li>
              Connecting Power Apps to SQL using stored procedures and Power
              Automate
            </li>
            <li>
              Building a simple inventory and order management application
            </li>
          </ul>
          <p>
            Let's take our first steps into this bigger world of
          ...
          <p>
            The first step is to set up an Azure SQL database. If you're new to
            Azure, you can sign up for a free trial which gives you plenty of
          ...
          <ol>
            ...
            <li>
              Take the default options for other settings and click "Create"
            </li>
          </ol>

          <h3>Configuring Server Firewall Rules</h3>
          <p>
            Once your server and database are created, you need to configure the
            firewall:
          </p>
          <ol>
            ...
            <li>
              Set "Allow Azure services and resources to access this server" to
              "Yes"
            </li>
            <li>
              Add your client IP address so you can connect from your desktop
            </li>
          </ol>
          ...
          <p>
            This firewall configuration is crucial as it allows Power Apps and
            Power Automate to connect to your database, while also letting you
            manage the database from your local computer.
          </p>

          <h2>Creating Your Database Structure</h2>
          <p>
            For this tutorial, we'll create a simple inventory management system
            with two tables: a Stock table for inventory items and an Orders
            table for tracking restock orders.
          </p>

          <h3>Connecting to Your Database</h3>
          <p>
            To manage your database, you'll need SQL Server Management Studio
            (SSMS):
          </p>
          <ol>
            <li>Download and install SQL Server Management Studio</li>
            <li>
              Connect to your database using the server name, admin username,
              and password
            </li>
          </ol>

          <h3>Creating Tables</h3>
          <p>We'll create two tables with a simple relationship:</p>
          ...
          <ul>
            <li>
              <strong>Stock Table:</strong> Stores inventory items with a name,
              description, and current stock level
            </li>
            <li>
              <strong>Orders Table:</strong> Tracks order history with
              references to stock items
            </li>
            <li>
              <strong>Relationships:</strong> The StockID in Orders refers to
              the ID in Stock, creating a one-to-many relationship
            </li>
          </ul>
          ...
          <p>
            Here's where things get interesting. Power Apps can't directly query
            multiple related SQL tables efficiently, so we'll create a stored
            procedure that joins the data for us:
          </p>

          <CodeBlock code={storedProcedureCode} language="sql" />

          <p>
            This stored procedure joins the Orders and Stock tables, giving us a
            flattened view that's perfect for displaying in Power Apps.
          </p>

          <h2>Connecting Power Apps to SQL Using Power Automate</h2>
          <p>
            Currently, Power Apps can't directly call SQL stored procedures.
            Instead, we'll use Power Automate as a bridge between our app and
            database.
          </p>

          <h3>Creating a Power Automate Flow</h3>
          <p>Here's how to set up the flow:</p>
          <ol>
            <li>Create a new instant flow with the PowerApps trigger</li>
            <li>
              Add an "Execute stored procedure" action from the SQL Server
              connector
            </li>
            <li>
              Configure it to connect to your database and call the "GetOrders"
              procedure
            </li>
          ...
          <p>
            The key here is the "Parse JSON" step. When you execute the flow for
            the first time, you can copy the JSON output from the stored
            procedure execution and use it to generate the schema automatically.
          </p>
          ...
          <p>Configure the button to call your flow and store the results:</p>

          <CodeBlock code={powerAppsGalleryCode} language="portal" />

          <h3>Displaying the Data</h3>
          <p>In your gallery, add labels to display the data:</p>
          <ol>
          ...
          <p>
            While we've covered the fundamentals of connecting Power Apps to
            SQL, a complete application would include:
          </p>
          <ul>
            <li>
              <strong>Create/Edit Forms:</strong> Screens to add and update
              stock items and orders
            </li>
            <li>
              <strong>Data Validation:</strong> Ensuring data integrity before
              saving to SQL
            </li>
            <li>
              <strong>Automatic Reordering:</strong> Triggering orders when
              stock levels fall below thresholds
            </li>
            <li>
              <strong>User Authentication:</strong> Ensuring only authorised
          ...
          <p>
            As you continue to develop with SQL and Power Apps, remember these
            important points:
          </p>
          <ol>
            <li>
              <strong>Use Stored Procedures:</strong> They're the key to working
              effectively with SQL, especially with related tables
            </li>
            <li>
              <strong>Power Automate is Your Bridge:</strong> Use it to connect
              Power Apps with SQL stored procedures
            </li>
            <li>
              <strong>Secure Your Database:</strong> Create specific database
              users for your apps rather than using admin credentials
            </li>
            <li>
              <strong>Plan Your Data Model:</strong> A well-designed database
              schema makes app development much easier
            </li>
            <li>
              <strong>Learn SQL Basics:</strong> Understanding SQL queries and
              joins will dramatically expand what you can do with Power Apps
            </li>
          </ol>

          <h2>Conclusion</h2>
          <p>
            Moving from SharePoint lists to SQL databases is a significant
            evolution in your Power Apps journey. While there's certainly a
            learning curve, the benefits are substantial:
          </p>
          <ul>
            <li>More sophisticated data relationships</li>
            <li>Better performance with large datasets</li>
            <li>
              Advanced data validation and business logic at the database level
            </li>
            <li>Enterprise-grade security and scalability</li>
          </ul>
          <p>
            This tutorial covered the core functionality of using SQL with Power
            Apps, but there's much more to explore. In future tutorials, we'll
            build on this foundation to create more complete applications with
            full CRUD (Create, Read, Update, Delete) capabilities.
          </p>

          <p>
            If you'd like to see more Power Apps development tutorials using
            SQL, please head over to the{" "}
            <a
              href="https://www.youtube.com/@ChinoDoesStuff/featured"
              rel="nofollow noreferrer noopener"
            >
