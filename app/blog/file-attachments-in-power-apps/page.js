...
Navigate(Home, ScreenTransition.None);`;
  ...
  const articleSchema = {
    ...
    description:
      "Learn how to implement file attachments in Power Apps the right way. This guide shows you how to use the new Power Apps V2 trigger to upload files directly to SharePoint document libraries.",
    mainEntityOfPage: {
      "@type": "WebPage",
      "@id":
    ...
    <>
      ...
      <div className={styles.blogContainer}>
        ...
        <div className={styles.blogContent}>
          <h2>Introduction</h2>
          <p>
            If you've ever tried to implement file attachments in Power Apps,
            you know it can be surprisingly challenging. You would expect
            attaching files to be a straightforward process, but the traditional
            approaches often involve complex workarounds and multiple steps for
            users.
          </p>
          <p>
            The good news? Microsoft has significantly improved this process
            with a new action in Power Automate that makes uploading documents
            to SharePoint document libraries much simpler and more efficient. In
          ...
          <ul>
            ...
            <li>
              How to connect everything together for a seamless user experience
            </li>
          ...
          <div className={styles.videoContainer}>
            <iframe
              src="https://www.youtube.com/embed/yC0W5am6M3Q?si=wlQ5FUzmmOka-J24"
              width="560"
              height="315"
              title="File attachments in power apps"
              allowFullScreen
            />
          </div>
          ...
          <p>
            Before diving into the implementation, let's understand why this
            method is superior to older approaches:
          </p>
          <ul>
            <li>
              <strong>Simpler Implementation:</strong> No need for complex
              Base64 encoding or JSON manipulation
            </li>
            <li>
              <strong>Multiple File Support:</strong> Easily handle multiple
              attachments in a single save operation
            </li>
            <li>
              <strong>Better User Experience:</strong> Users can attach all
              files and save the record in one step
            </li>
            <li>
              <strong>Document Library Storage:</strong> Files are saved to a
              SharePoint document library instead of list attachments, providing
              better management options
            </li>
            <li>
              <strong>Relationship Preservation:</strong> Maintains the
              relationship between files and records
            </li>
          </ul>

          <h2>Step 1: Add the File Attachment Control</h2>
          <p>
            One quirk of Power Apps is that the file attachment control isn't
            directly available in the insert menu. Here's how to add it to your
            app:
          </p>
          <ol>
            <li>Insert a Form control onto your screen</li>
            <li>
              Connect the form to any SharePoint list (it doesn't matter which
              one)
            </li>
            <li>Find the attachment control in the form</li>
            <li>
              Cut the attachment control (not the entire data card, just the
              control)
            </li>
            <li>Delete the form</li>
            <li>Paste the attachment control directly onto your screen</li>
          </ol>
          <p>After pasting, you'll need to fix some errors in the control:</p>

          <CodeBlock code={attachmentControlCode} language="power apps" />

          <p>
            The MaxAttachments and MaxAttachmentSize properties are particularly
            important to configure based on your requirements.
          </p>

          <h2>Step 2: Create the Power Automate Flow</h2>
          <p>
            Next, we'll create a flow that receives files from Power Apps and
            saves them to a SharePoint document library:
          </p>

          <h3>Set Up the Document Library</h3>
          <p>
            In SharePoint, create a document library to store your attachments.
            Add a Number column called "ParentId" - this will store the ID of
            the related record from your list.
          </p>
          ...
          <ol>
            <li>
              <strong>Trigger:</strong> Use the "Power Apps (V2)" trigger - this
              is crucial as it supports direct file inputs
            </li>
          </ol>
          ...
          <ol start="2">
            <li>
              <strong>Create File in SharePoint:</strong> This action saves the
              file to your document library
            </li>
            <li>
              <strong>Update File Properties:</strong> This sets the ParentId
              property to associate the file with your record
            </li>
            <li>
              <strong>Respond to Power Apps:</strong> This completes the flow
              and returns control to your app
            </li>
          </ol>
          ...
          <p>
            Save your flow with a descriptive name like "Save File to
            SharePoint".
          </p>

          <h2>Step 3: Connect Power Apps to Your Flow</h2>
          <p>
            Now, we need to modify our app's save button to handle both the
            record creation and file uploads:
          </p>
          ...
          <ol>
            ...
            <li>
              We capture the ID of the newly created record in a variable
              (RecId)
            </li>
            <li>
              We use ForAll() to loop through each attachment in the
              FileAttachments control
            </li>
            <li>
              For each attachment, we call our flow, passing both the record ID
              and the file details
            </li>
            <li>
              Finally, we reset our form controls and navigate back to the home
              screen
            </li>
          </ol>

          <h2>How It All Works Together</h2>
          <p>When a user fills out your form and attaches files:</p>
          <ol>
            ...
            <li>
              The flow saves each file to the document library and sets the
              ParentId property
            </li>
            <li>The files are now linked to your record by the ParentId</li>
          </ol>
          <p>
            This entire process happens seamlessly, and the user only needs to
            click Save once.
          </p>

          <h2>Viewing Attached Files</h2>
          <p>To display files associated with a record, you'll need to:</p>
          <ol>
            <li>Create a gallery control on your view screen</li>
            <li>Connect it to your document library</li>
            <li>
              Filter the gallery to show only files where ParentId matches your
              current record's ID
            </li>
            <li>Add an icon or button to open each file</li>
          </ol>
          <p>
            The formula for the gallery's Items property would be something
            like:
          </p>
          <pre>
            <code>
          ...
          <p>When implementing file attachments in Power Apps:</p>
          <ul>
            <li>
              <strong>Handle Large Files:</strong> Be aware of SharePoint's file
              size limits and set MaxAttachmentSize accordingly
            </li>
            <li>
              <strong>Add Progress Indicators:</strong> For better user
              experience, add a loading spinner during file uploads
            </li>
            <li>
              <strong>Error Handling:</strong> Consider adding error handling to
              deal with upload failures
            </li>
            <li>
              <strong>File Types:</strong> You can filter allowed file types
              through the attachment control's properties
            </li>
            <li>
              <strong>Security:</strong> Ensure users have appropriate
              permissions to the SharePoint document library
            </li>
          </ul>

          <h2>Advanced Customisations</h2>
          <p>This approach can be extended in several ways:</p>
          <ul>
          ...
          <h2>Conclusion</h2>
          <p>
            With Microsoft's improved Power Apps V2 trigger in Power Automate,
            handling file attachments in Power Apps has become significantly
            easier. This approach provides a clean, user-friendly way to upload
            files to SharePoint and maintain their relationship with your app's
            data.
          </p>
          <p>
            By using a document library instead of list attachments, you gain
            all the benefits of SharePoint's document management capabilities
            while providing a seamless experience for your users.
          </p>
          <p>
            This method eliminates much of the complexity that was previously
            required for file uploads in Power Apps, making your apps more
